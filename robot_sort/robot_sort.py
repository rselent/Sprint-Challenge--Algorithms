class SortingRobot:
	...
	def sort(self):
		...
		"""
		# how I would PREFER to do this... ¬__¬
		# (this passes all stretches, btw... timsort ftw)
		# x = self._list
		# x.sort()

		self.set_light_on()						# use light as a completion flag
		while self.light_is_on():				# if light is on, run robot run
			...
			while self.can_move_right():		# general in-place movement:
				...
				if self.compare_item() == 1:	# compare 'held' item to next item, 
					...
					self.swap_item()			#	swap,
					self.move_right()			#	and move right again / keep checking

			while self.can_move_left() and self.light_is_on():
				self.move_left()				# if light is on (set above),
												# keep moving left, to beginning of list,
												# to either start again, or end
...
if __name__ == "__main__":
	...
	l = [15, 41, 58, 49, 26, 4, 28, 8, 61, 60, 65, 21, 78, 14, 35, 
		 90, 54, 5, 0, 87, 82, 96, 43, 92, 62, 97, 69, 94, 99, 93, 
		 76, 47, 2, 88, 51, 40, 95, 6, 23, 81, 30, 19, 25, 91, 18, 
		 68, 71, 9, 66, 1, 45, 33, 3, 72, 16, 85, 27, 59, 64, 39, 
		 32, 24, 38, 84, 44, 80, 11, 73, 42, 20, 10, 29, 22, 98, 
		 17, 48, 52, 67, 53, 74, 77, 37, 63, 31, 7, 75, 36, 89, 
